# Privacy Policy

**Effective Date:** September 6, 2025  
**Last Updated:** September 6, 2025

## Introduction

WealthTrak ("we," "our," or "us") is committed to protecting your privacy and the confidentiality of your financial information. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our financial tracking application and related services (the "Service").

By using WealthTrak, you agree to the collection and use of information in accordance with this Privacy Policy.

## Information We Collect

### Personal Information
We collect the following types of personal information:

**Account Information:**
- Email address (required for account creation)
- Password (encrypted and securely stored)
- Profile information (name, preferences)
- Authentication tokens and session data

**Financial Data:**
- Account names and types (assets, liabilities)
- Account balances and historical snapshots
- Financial goals and progress tracking
- Net worth calculations and trends
- Currency preferences and settings

**Usage Information:**
- Device information (browser type, operating system)
- IP address and location data
- Usage patterns and feature interactions
- Error logs and performance metrics
- Session duration and frequency of use

**Communication Data:**
- Support requests and correspondence
- Feedback and survey responses
- Marketing communication preferences

## How We Use Your Information

We use your information for the following purposes:

### Service Provision
- Provide and maintain the WealthTrak application
- Process and store your financial data securely
- Generate financial reports, charts, and analytics
- Synchronize data across your devices
- Provide customer support and technical assistance

### Security and Fraud Prevention
- Verify your identity and authenticate access
- Detect and prevent fraudulent activities
- Monitor for security threats and vulnerabilities
- Implement data loss prevention measures
- Maintain audit logs for security purposes

### Service Improvement
- Analyze usage patterns to improve functionality
- Develop new features and capabilities
- Optimize application performance
- Conduct research and analytics
- Personalize your user experience

### Legal Compliance
- Comply with applicable laws and regulations
- Respond to legal requests and court orders
- Enforce our Terms of Service
- Protect our rights and property
- Resolve disputes and prevent harm

## Information Sharing and Disclosure

### We Do Not Sell Your Data
We do not sell, trade, or rent your personal financial information to third parties for commercial purposes.

### Limited Sharing Scenarios
We may share your information only in the following circumstances:

**Service Providers:**
- Cloud hosting providers (Supabase/PostgreSQL)
- Authentication services
- Email service providers
- Analytics and monitoring tools
- Customer support platforms

**Legal Requirements:**
- Court orders, subpoenas, or legal process
- Government investigations or regulatory requests
- Protection of rights, property, or safety
- Prevention of fraud or illegal activities
- Compliance with financial regulations

**Business Transfers:**
- Mergers, acquisitions, or asset sales
- Bankruptcy or insolvency proceedings
- Corporate restructuring events

**With Your Consent:**
- Explicit permission for specific purposes
- Integration with third-party financial services
- Sharing with authorized family members or advisors

## Data Security

### Encryption and Protection
We implement industry-standard security measures:

**Data Encryption:**
- TLS/HTTPS encryption for data in transit
- AES-256 encryption for sensitive data at rest
- Encrypted database storage with Supabase
- Secure key management and rotation

**Access Controls:**
- Multi-factor authentication options
- Role-based access controls
- Session management and timeout
- IP-based access restrictions
- Regular security audits and penetration testing

**Infrastructure Security:**
- SOC 2 Type II compliant hosting
- Regular security updates and patches
- Intrusion detection and monitoring
- Backup and disaster recovery procedures
- Data center physical security measures

### Data Retention
- Account data: Retained while your account is active
- Financial data: Retained for 7 years after account closure (regulatory compliance)
- Usage logs: Retained for 90 days for security purposes
- Support communications: Retained for 3 years
- Marketing data: Retained until you withdraw consent

## Your Privacy Rights

### Access and Control
You have the following rights regarding your personal information:

**Right to Access:** Request copies of your personal data
**Right to Rectification:** Correct inaccurate or incomplete information
**Right to Erasure:** Request deletion of your personal data (subject to legal requirements)
**Right to Portability:** Export your data in a structured format
**Right to Restrict Processing:** Limit how we use your information
**Right to Object:** Opt-out of certain data processing activities

### Account Management
- Update your profile and preferences at any time
- Download your financial data via export features
- Delete individual transactions or accounts
- Close your account and request data deletion
- Manage email and communication preferences

### Data Portability
We provide tools to export your data in standard formats:
- CSV export for all financial data
- JSON export for complete account information
- Excel-compatible formats for analysis
- API access for automated data retrieval

## Cookies and Tracking

### Essential Cookies
We use necessary cookies for:
- User authentication and sessions
- Security and fraud prevention
- Application functionality
- User preferences and settings

### Analytics Cookies
With your consent, we use analytics cookies for:
- Usage statistics and performance monitoring
- Feature adoption and user behavior analysis
- Error tracking and debugging
- Application optimization

### Cookie Management
- You can control cookies through your browser settings
- Disabling essential cookies may affect functionality
- We respect "Do Not Track" browser signals
- Cookie preferences can be updated in your account settings

## International Data Transfers

### Data Processing Locations
Your data may be processed in:
- United States (primary data centers)
- European Union (backup and redundancy)
- Other countries where our service providers operate

### Transfer Safeguards
We ensure appropriate safeguards for international transfers:
- Standard Contractual Clauses (SCCs)
- Adequacy decisions by relevant authorities
- Binding Corporate Rules where applicable
- Your explicit consent for specific transfers

## Children's Privacy

WealthTrak is not intended for individuals under 18 years of age. We do not knowingly collect personal information from children under 18. If we discover that a child under 18 has provided us with personal information, we will delete such information immediately.

## Third-Party Services

### Integration Partners
We may integrate with third-party financial services:
- Bank account aggregation services
- Credit monitoring services
- Investment tracking platforms
- Tax preparation software

### Third-Party Privacy Policies
When using integrated services, you are subject to their privacy policies. We encourage you to review the privacy policies of any third-party services you connect to WealthTrak.

## California Privacy Rights (CCPA)

### California Residents
If you are a California resident, you have additional rights:

**Right to Know:** Categories and specific pieces of personal information collected
**Right to Delete:** Deletion of personal information (subject to exceptions)
**Right to Opt-Out:** Sale of personal information (we do not sell data)
**Right to Non-Discrimination:** Equal service regardless of privacy choices

### Exercising CCPA Rights
To exercise your rights:
- Email us at customerservice@techbrov.com
- Use our in-app privacy request form
- Call our privacy hotline at [phone number]
- Response time: 45 days (extendable by 45 days)

## European Privacy Rights (GDPR)

### Legal Basis for Processing
We process your data based on:
- **Contract Performance:** Providing WealthTrak services
- **Legitimate Interest:** Security, fraud prevention, and service improvement
- **Consent:** Marketing communications and optional features
- **Legal Obligation:** Compliance with financial regulations

### Data Protection Officer
For GDPR-related inquiries, contact our Data Protection Officer:
- Email: customerservice@techbrov.com
- Address: [Physical address in EU]

### Supervisory Authority
You have the right to lodge a complaint with your local data protection authority.

## Changes to This Privacy Policy

We may update this Privacy Policy periodically to reflect changes in our practices or legal requirements. We will notify you of material changes by:

- Email notification to your registered address
- In-app notifications upon login
- Posting updates on our website
- 30-day advance notice for material changes

Continued use of WealthTrak after changes constitutes acceptance of the updated Privacy Policy.

## Contact Information

### Privacy Questions
For privacy-related questions or concerns:

**Email:** customerservice@techbrov.com
**Phone:** [Privacy Hotline Number]
**Mail:**
Techbrov Privacy Team
[Physical Address]
[City, State, ZIP Code]

### Data Protection Requests
To exercise your privacy rights:
- Use our automated privacy request portal
- Email customerservice@techbrov.com
- Include your full name and account email
- Specify the type of request (access, deletion, etc.)
- Allow up to 30 days for processing

### Security Incidents
To report security concerns:
- Email: customerservice@techbrov.com
- Use our responsible disclosure program
- Report through our bug bounty platform

---

**This Privacy Policy is effective as of the date stated above and applies to all users of WealthTrak financial tracking services.**

© 2025 Techbrov. All rights reserved.